...
import MarketTicker, { TickerItem } from "@/components/MarketTicker";
import AlertHistory from "@/components/AlertHistory";
import { calculateValuation, formatCurrency } from "@/lib/valuation";
import { supabase } from '@/lib/supabase';
import { generateCombinedTickerItems, filterTopTickerItems, saveAlert } from "@/lib/ticker-utils";
import { subscribeToAllUpdates } from "@/lib/realtime";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Set up WebSocket realtime subscriptions
    const unsubscribe = subscribeToAllUpdates((newAlert) => {
      setTickerItems(prev => {
        // Add new alert to the beginning
        const updated = [newAlert, ...prev];
        // Keep only top 15
        return filterTopTickerItems(updated, 15);
      });
    });
  ...
  const fetchStocks = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        // Generate ticker items from stock data + news + dividends
        const allTickerItems = await generateCombinedTickerItems(formattedStocks);
        const topItems = filterTopTickerItems(allTickerItems, 15);
        setTickerItems(topItems);

        // Auto-save top alerts to database for history
        for (const item of topItems.slice(0, 10)) {
          const stock = formattedStocks.find(s => s.symbol === item.symbol);
          if (stock) {
            await saveAlert({
              stock_id: stock.id,
              symbol: item.symbol,
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      {tickerItems.length > 0 && (
        <MarketTicker
          items={tickerItems}
          onItemClick={(symbol) => setSelectedStock(symbol)}
          pauseOnHover={true}
        />
      )}
      ...
      <div className="p-4 md:p-8 pb-32">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <AlertHistory onAlertClick={(symbol) => setSelectedStock(symbol)} />
